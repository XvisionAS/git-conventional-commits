...
exports.builder = function (yargs) {
  ...
  yargs.options('days', {
    alias: 'd',
    type: 'number',
    desc: 'Number of days to get the log for',
    default: 2
  })
}
...
exports.handler = async function (argv) {
    ...
    startDate.setDate(todayDate.getDate() - argv.days)

    const days = getDaysArray(startDate, todayDate)
    for (const day of days) {
      const dayValue = day.setHours(0, 0, 0, 0)
      const dayChangelogCommits = changelogCommits.filter(
        commit => commit.date.setHours(0,0,0,0) == dayValue
      )
      if (dayChangelogCommits.length) {
        let firstChangelogCommit = dayChangelogCommits[0].hash;
        let lastChangelogCommit = dayChangelogCommits.slice(-1)[0].hash;
        let changelogMarkdown = changelogGenerator.generateMarkdown(
          firstChangelogCommit, lastChangelogCommit, 
          releaseName, dayChangelogCommits, argv.markdown, day);  
        if (argv.file) {
